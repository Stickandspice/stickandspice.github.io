...
function loadProducts(filterCategory = 'all') {
    ...
    // Create product cards for jars
    jars.forEach(product => {
    ...
    if (bundleGrid) {
        ...
        if (bundles.length > 0) {
            bundles.forEach(product => {
                ...
                `;
                bundleGrid.appendChild(productCard);
            });
        } else {
...
function attachAddToCartListeners() {
    document.querySelectorAll('.add-to-cart-btn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            // Placeholder - will be replaced with Stripe Checkout integration
            alert(`Great choice! "${productName}" will be added to your cart.\n\nStripe checkout integration coming soon!`);
            addToCartAnimation(this);
            
            // TODO: Integrate with Stripe Checkout
            // redirectToStripeCheckout(productId, productName, productPrice);
        });
    });
}
    ...
    if (cart.length === 0) {
        ...
        cartItemsContainer.innerHTML = cart.map(item => `
            <div style="display: flex; gap: 1rem; padding: 1rem; background: #f8f9fa; border-radius: 10px; margin-bottom: 1rem; align-items: center;">
                ...
                <div style="flex: 1;">
                    <h4 style="margin: 0 0 0.25rem 0; color: #2d3436; font-size: 1rem;">${item.name}</h4>
                    <p style="margin: 0; color: #cc567e; font-weight: 600;">$${item.price.toFixed(2)}</p>
                </div>
                <div style="display: flex; align-items: center; gap: 0.5rem;">
                    ...
                    <span style="min-width: 30px; text-align: center; font-weight: 600; color: #2d3436;">${item.quantity}</span>
                    <button onclick="updateQuantity('${item.id}', 1)" style="background: white; border: 2px solid #e0e0e0; width: 30px; height: 30px; border-radius: 5px; cursor: pointer; font-weight: bold; color: #636e72; transition: all 0.3s ease;" onmouseover="this.style.borderColor='#cc567e'; this.style.color='#cc567e'" onmouseout="this.style.borderColor='#e0e0e0'; this.style.color='#636e72'">
